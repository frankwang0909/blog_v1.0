...
## 在线图形工具(https://regexper.com/)：可以在github上下载到本地，跑在本地，运行速度更快
匹配邮箱：
...
## 修饰符：
1. g(global):表示全局匹配，不添加则默认匹配到第一个字符串。
2. i(ignore case):表示忽略大小写,不添加则默认大小写敏感。
3. m(multiple lines):表示多行匹配。

## 原意文本字符 和元字符：
1. 原意文本字符：表示字符的原本含义，如'a'表示小写字母a， '1' 表示数字1。
2. 元字符：在正则表达式中有特殊含义的非字母字符，如星号*，问号？等。
3. 常用元字符及其含义：
	1）[]:构建字符类，范围类等
	2）{}:量词，表示字符的个数。
	3）():构建分组
...
## 字符类：
通常，正则表达式一个字符对应字符串一个字符，如/a/对应字母a。
...
## 贪婪模式与非贪婪模式：
1.贪婪模式：正则表达式会尽可能多地去匹配字符。
例如，下面的正则表达式，匹配3到6个数字的字符串，默认会尽可能多地去匹配。
`'b12345678A5879e123'.match(/\d{3,6}/g)`
>>>`["123456", "5879", "123"]`

2. 非贪婪模式：在量词后加一个问号, 尽可能少地去匹配字符串。
`'b12345678A5879e123'.match(/\d{3,6}?/g)`
>>>`["123", "456", "587", "123"]`

## 分组：括号“()”把字符串变成分组，使量词作用于分组。
示例代码：
1.`'abcabcabccc'.match(/abc{3}/g); `
>>>`["abccc"]`

2.`'abcabcabccc'.match(/(abc){3}/g);`
>>>`["abcabcabc"]`

### 逻辑或：竖线 “|” 作用于分组，可以表示多个中的某一个均可。
1. `'whatwhaowho'.match(/wh(at|o)/g)`
>>>["what", "who"]
2. `'whatwhaowhowhy'.match(/wh(at|o|y)/g)`
>>>["what", "who", "why"]

### 捕获分组，反向引用
想要把如下日期格式化：2016-01-20 => 01/20/2016；
可以这样写`'2016-01-20'.replace(/(\d{4})-(\d{2})-(\d{2})/g, "$2/$3/$1")`
>>>"01/20/2016"
dollar符"$"加上一个数字，数字对应的是分组的序号，第一个分组的内容用“$1”来捕获，第二分组的内容用“$2”来捕获，以此类推。

### 忽略分组：分组内加上"?:"表示忽略该分组，而捕获该组的内容。
	`'2016-01-20'.replace(/(?:\d{4})-(\d{2})-(\d{2})/g, "$1/$2")`
	>>>"01/20"

## 断言：
### 正则表达式从文本头部向文本尾部开始解析，文本尾部方向称为“前”，反之，为“后”
### 前瞻与后顾：正则表达式匹配到规则的时候，向前检查是否符合断言，称为"前瞻"。向后检查是否符合断言则称为"后顾"。JavaScript不支持后顾。
### 正向/肯定前瞻：向前符合断言。exp(?=assert)
	`'5a68cz7'.match(/\d(?=[a-z])/g);`
	>>>["5", "8"]

### 负向/否定前瞻：向前不符合断言。exp(?!assert)
	`'5a68cz7'.match(/\d(?=[a-z])/g);`
	>>>["6", "7"]

## RegExp对象属性
1. global: 是否全文搜索（只读属性）;
2. ignoreCase：是否大小写敏感（只读属性）;
3. multline: 是否多行（只读属性）;
4. source：正则表达式文本内容；（只读属性）;
5. lastIndex: 当前表达式匹配内容的最后一个字符的下一个位置。   
...
## RegExp的方法：test(), exec();
### RegExp.test(string): 测试某个字符串是否能够匹配该正则表达式。参数为字符串；返回值为布尔值。
	`var reg1 = /\d/, reg2 = /\d/g;`
	`reg1.test('a12ab');`
	>>>true
...
## String的一些正则操作(查找、替换、切分等)方法：match()、search()、replace()、split();
### string.match(RegExp)： 
